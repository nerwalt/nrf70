...
impl<BUS: Bus> Rpu<BUS> {
    ...
    pub async fn update_cached_receive_buffer(
        ...
        // We've read the data into cache, so now release the RX buffer in the PRU
        self.release_receive_buffer(descriptor_identifier).await?;

        Ok(())
    }

    pub async fn release_receive_buffer(&mut self, descriptor_id: usize) -> Result<(), Error> {
